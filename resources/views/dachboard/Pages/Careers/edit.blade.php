...
@section('content')
    <div class="container">
        <form class="edit_career main_form" method="POST" action="{{ route('careers.update', $career['id']) }}"
            enctype="multipart/form-data">
            @csrf
            @method('PUT') <!-- Use the PUT method for updating -->
            <div class="mb-3 w-100">
                <label for="careerNameEn" class="form-label">English Career Title</label>
                <input type="text" class="form-control" id="careerNameEn" name="name_en" value="{{ $career['name_en'] }}">
...
@push('scripts')
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            const form = document.querySelector('.edit_career');
